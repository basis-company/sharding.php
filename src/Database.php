...
class Database implements Crud
{
    ...
    {
        if ($this->propertyExists($class, 'id') && (!array_key_exists('id', $data) || !$data['id'])) {
    ...
    private function propertyExists(string $class, string $property): bool
    {
        $model = $this->schema->getClassModel($class);
        return $model->hasProperty('id');
    }
}
